...
function getSubjectsStatistics(students) {
  return students.map(student => {
    const average = Object.values(student.grades).reduce((sum, grade) => sum + grade, 0) / Object.keys(student.grades).length;
    return {
...
function getHighAchievers(students, gradeThreshold) {
  return students
    .filter(student => getSubjectsStatistics([student])[0].average > gradeThreshold)
    .map(student => `${student.firstName} ${student.lastName}`);
}

function getMultiAchivers(students, threshold) {
  return students
    .filter(student => Object.values(student.grades).filter(grade => grade > threshold).length >= 2)
    .map(student => {
